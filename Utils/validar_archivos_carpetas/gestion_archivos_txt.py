...
def abrir_txt(ruta: str, modo: str = "r"):
    try:
        ...
        agregar_log(f"[SUCCESS] Archivo abierto desde Python: {ruta} | modo={modo}")
        return f
    except Exception as e:
        agregar_log(f"[ERROR] No se pudo abrir el archivo '{ruta}' en modo '{modo}': {e}")
        return None
...
def cerrar_todos_txt(nombre_archivo: str = "GNSS.txt") -> Optional[bool]:
    try:
        agregar_log(f"[INFO] Buscando procesos que puedan tener abierto '{nombre_archivo}'...")
        ...
        if not lista_procesos:
            agregar_log("[INFO] No se encontraron editores de texto abiertos.")
            return None
        ...
        for proc_name in lista_procesos:
            try:
                ...
                if resultado.returncode == 0:
                    agregar_log(f"[SUCCESS] Proceso cerrado: {proc_name}")
                    exito = True
                else:
                    agregar_log(f"[ERROR] No se pudo cerrar {proc_name}: {resultado.stderr.strip()}")
                    error = True
            except Exception as e:
                agregar_log(f"[ERROR] Excepción al intentar cerrar {proc_name}: {e}")
                error = True
        ...
        agregar_log(f"[EXCEPTION] Error inesperado en 'cerrar_txt_externo': {e}")
        return False
