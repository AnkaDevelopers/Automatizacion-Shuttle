...
def validar_carpetacion(ruta: str, requeridas: List[str]) -> Optional[Dict[str, str]]:
    try:
        ...
        if not p.exists() or not p.is_dir():
            agregar_log(f"[ERROR] La ruta '{ruta}' no existe o no es un directorio válido.")
            return None
        agregar_log(f"[INFO] Ruta válida: {ruta}")
        ...
        agregar_log(f"[DEBUG] Subcarpetas encontradas: {list(subdirs.keys())}")

        # ------------------------------------------------------------------
        # 3. Verificar la presencia de carpetas requeridas
        # ------------------------------------------------------------------
        if not all(r.lower() in subdirs for r in requeridas):
            agregar_log(f"[ERROR] Faltan carpetas requeridas. Se esperaban: {requeridas}")
            return None
        agregar_log("[INFO] Todas las carpetas requeridas están presentes.")

        # ------------------------------------------------------------------
        # 4. Devolver rutas absolutas/resueltas
        # ------------------------------------------------------------------
        resultado = {r: str(subdirs[r.lower()].resolve()) for r in requeridas}
        agregar_log(f"[SUCCESS] Estructura de carpetas validada correctamente: {resultado}")
        return resultado
        ...
        # ------------------------------------------------------------------
        agregar_log(f"[EXCEPTION] Error inesperado al validar la ruta '{ruta}': {e}")
        return None
